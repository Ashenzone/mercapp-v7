...
ALTER TABLE users ADD COLUMN IF NOT EXISTS balance_cents BIGINT NOT NULL DEFAULT 500000; -- R$5.000 pra comecar
CREATE UNIQUE INDEX IF NOT EXISTS idx_users_nickname ON users(nickname) WHERE nickname IS NOT NULL;
ALTER TABLE users ALTER COLUMN balance_cents SET DEFAULT 500000;
...
-- quem ja tinha criado conta com saldo 0 (padrao antigo, antes do saldo inicial
-- de R$5.000) recebe o valor uma unica vez -- guardado pra nao aplicar de novo
-- a cada deploy (o schema roda toda vez que o servidor sobe)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM game_config WHERE key = 'saldo_inicial_aplicado') THEN
    UPDATE users SET balance_cents = balance_cents + 500000 WHERE balance_cents = 0;
    INSERT INTO game_config (key, value_cents) VALUES ('saldo_inicial_aplicado', 1)
    ON CONFLICT (key) DO NOTHING;
  END IF;
END $$;
